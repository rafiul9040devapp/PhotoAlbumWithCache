...
@HiltViewModel
class PhotoViewModel @Inject constructor(private val photoRepository: PhotoRepositoryImpl): ViewModel()  {

    private val _photos = MutableStateFlow<ApiState<List<ResponsePhotoItem>>>(ApiState.Loading)
    val photos: StateFlow<ApiState<List<ResponsePhotoItem>>> get() = _photos
    ...
    private fun fetchAllPhotos() {
        viewModelScope.launch {
            photoRepository.getAllPhotosFromRepo().collect { apiState ->
                _photos.value = apiState
            }
        }
    }
}